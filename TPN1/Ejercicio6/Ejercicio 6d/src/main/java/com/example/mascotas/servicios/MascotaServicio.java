...
public class MascotaServicio {
    ...
    public void agregarMascota(MultipartFile archivo, String idUsuario, String nombre, Sexo sexo, Tipo tipo) throws ErrorServicio {
        if (idUsuario == null || idUsuario.isBlank()) throw new ErrorServicio("Debe indicar el usuario");
        Usuario usuario = usuarioRepositorio.findById(idUsuario)
                .orElseThrow(() -> new ErrorServicio("No se encontro el usuario solicitado"));
    ...
    public Mascota buscarMascota(String idMascota) throws ErrorServicio {
        try {
            ...
            if (optional.isEmpty() || optional.get().getBaja() != null) throw new ErrorServicio("No se encuentra la mascota indicada");
            return optional.get();
    ...
    public void validar(String nombre, Sexo sexo) throws ErrorServicio {
        if (nombre == null || !nombre.trim().matches("[A-Za-zÁÉÍÓÚáéíóúÑñ' -]{2,60}")) {
            throw new ErrorServicio("El nombre de la mascota debe tener entre 2 y 60 caracteres válidos");
        }
        if (sexo == null) {
